...
async function syncApis(env) {
  ...
  try {
    ...
    const response = await fetch(env.SYNC_URL || 'https://api.apis.guru/v2/list.json', {
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`HTTP error fetching API list: ${response.status} ${response.statusText}`);
    }
    ...
    for (let i = 0; i < entries.length; i += batchSize) {
      ...
      try {
        ...
        const statements = batch.map(([name, apiEntry]) => {
          ...
          const categories = (info['x-apisguru-categories'] || []).map(c => c.toLowerCase());
          ...
          const externalUrl = externalDocs.url || 
                            contact.url || 
        ...
        console.log(`Processed batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(entries.length / batchSize)} (${batch.length} APIs)`);
    ...
    console.log(`Sync completed in ${duration}ms. Processed: ${totalProcessed}, Errors: ${totalErrors}`);
    ...
    try {
      ...
      `).bind(`Last sync: ${new Date().toISOString()}, Processed: ${totalProcessed}, Errors: ${totalErrors}`).run();
    } catch (metaError) {
